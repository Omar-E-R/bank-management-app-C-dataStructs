...
struct account_t
{
	...
	char uuid_account[UUID_SIZE];

	char uuid_activity[UUID_SIZE];

	char iban[IBAN_SIZE];

	char account_no[ACCOUNT_NO_SIZE];
...
struct individual_t
{
	...
	int zip_code;
	char *address_no2;
	char mobile_phone[PHONE_SIZE];
	char home_phone[PHONE_SIZE];
	char *id_card_no;
	char joineddate[DATE_JOIN_SIZE];
	size_t flag;
...
struct agency_t
{
	...
	char uuid_agency[UUID_SIZE];

	char agency_id[AGENCY_ID_SIZE];
...
int bank_individual_reset_login(individual_t *individual, login_t *login, char*
newpass);
